                    ...
                    ->arrayPrototype()
                        ->children()
                            ->scalarNode('target')->isRequired() // @legacy SF < 7.2 (string node introduced in Symfony 7.2)
                                ->validate()
                                    ->ifTrue(static fn (mixed $value) => !\is_string($value) || (!preg_match(DeployRsyncCommand::TARGET_FILE_REGEX, $value) && !preg_match(DeployRsyncCommand::TARGET_SSH_REGEX, $value)))
                            ...
                            ->scalarNode('ignore_file') // @legacy SF < 7.2 (string node introduced in Symfony 7.2)
                                ->validate()
                                    ->ifTrue(static fn (mixed $value) => !\is_string($value) && null !== $value)
